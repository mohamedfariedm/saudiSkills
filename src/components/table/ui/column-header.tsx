...
export function DataTableColumnHeader<TData, TValue>({
  ...
  if (!column.getCanSort() && !column.getCanHide()) {
    return (
      <h2 className={cn("text-primary-700 text-center text-sm", className)}>
        {title}
      </h2>
    );
  }
    ...
    <div className={cn("flex items-center gap-2", className)}>
      ...
      >
        <SelectTrigger
          ...
          className="-ml-3 h-8 w-fit border-none !text-primary-600 text-sm hover:bg-primary-100  data-[state=open]:bg-accent [&>svg:last-child]:hidden mx-auto"
        >
          {title}
          <SelectIcon asChild className="">
