  ...
  return (
    <Collapsible open={activeSubmenu === index}>
      <CollapsibleContent className="CollapsibleContent">
        <ul className="sub-menu  space-y-4 relative before:absolute before:left-4 before:top-0  before:h-[calc(100%-5px)]  before:w-[3px] before:bg-theme-color/10 before:rounded">
          {item.child?.map((subItem: any, j: number) => (
            <li
              className={cn(
                ...
                {
                  "before:bg-theme-color first:before:h-[calc(100%-16px)]  before:h-full":
